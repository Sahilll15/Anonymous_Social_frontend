...
const CommentsPage = () => {
  ...
  const fetchPostByID=async()=>{
    try {
      ...
      const authToken = localStorage.getItem('auth');
        const response=await fetch(`https://anonymous-social-bt77.onrender.com/api/v1/posts/getpost/${postID}`,{
          method:'GET',
          headers:{
  ...
  const fetchCOmmentsByPostID=async()=>{
    ...
    try {
      ...
      const authToken = localStorage.getItem('auth');
        const response=await fetch(`https://anonymous-social-bt77.onrender.com/api/v1/comments/getcomments/${postID}`,{
          method:'GET',
          headers:{
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}`
